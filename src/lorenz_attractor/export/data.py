...
class DataExporter:
    ...
    def export_csv(
        ...
        # Create DataFrame
        df = pd.DataFrame(
            {'time': result.time, 'x': result.x, 'y': result.y, 'z': result.z}
        )
    ...
    def export_multiple_results(
        self, results: List[SimulationResult], base_filename: str, format: str = 'csv'
    ) -> List[str]:
        """
    ...
    def export_parameter_sweep_summary(
        self,
    ...
    def export_bifurcation_data(
        self, bifurcation_data: Dict[str, Any], filename: str
    ) -> str:
        """
        ...
        for param_val, attractor in zip(param_values, attractors):
            for point in attractor:
                all_data.append(
                    {param_name: param_val, 'x': point[0], 'y': point[1], 'z': point[2]}
                )
    ...
    def create_data_package(
        ...
        for i, result in enumerate(results):
            ...
            csv_file = self.export_csv(
                result, package_path / 'data' / f"{base_name}.csv"
            )
            json_file = self.export_json(
                result, package_path / 'data' / f"{base_name}.json"
            )
            numpy_file = self.export_numpy(
                result, package_path / 'data' / f"{base_name}.npz"
            )
            ...
            if include_plots:
                ...
                fig.savefig(
                    package_path / 'plots' / f"{base_name}_3d.png",
                    dpi=300,
                    bbox_inches='tight',
                )

                # 2D projections
                fig = plotter.plot_2d_projections(
                    result, title=f"Simulation {i} - Projections"
                )
                fig.savefig(
                    package_path / 'plots' / f"{base_name}_2d.png",
                    dpi=300,
                    bbox_inches='tight',
                )
