...
Examples:
  ...
  lorenz-sim bifurcation --parameter rho --range 0 50 --points 1000
    ...
    sim_parser.add_argument(
        '--x0', type=float, default=1.0, help='Initial X coordinate'
    )
    sim_parser.add_argument(
        '--y0', type=float, default=1.0, help='Initial Y coordinate'
    )
    sim_parser.add_argument(
        '--z0', type=float, default=1.0, help='Initial Z coordinate'
    )
    ...
    sim_parser.add_argument(
        '--method',
        choices=['euler', 'rk4', 'adaptive', 'dopri5'],
        default='rk4',
    ...
    sim_parser.add_argument(
        '--random-ic', action='store_true', help='Use random initial conditions'
    )
    sim_parser.add_argument(
        '--true-random', action='store_true', help='Use true random initial conditions'
    )
    ...
    realtime_parser.add_argument(
        '--method',
        choices=['matplotlib', 'pygame', 'moderngl'],
        default='matplotlib',
        help='Visualization method',
    )
    realtime_parser.add_argument(
        '--sigma', type=float, default=10.0, help='Sigma parameter'
    )
    realtime_parser.add_argument(
        '--rho', type=float, default=28.0, help='Rho parameter'
    )
    realtime_parser.add_argument(
        '--beta', type=float, default=8 / 3, help='Beta parameter'
    )
    realtime_parser.add_argument(
        '--x0', type=float, default=1.0, help='Initial X coordinate'
    )
    ...
    sweep_parser.add_argument(
        '--parameter',
        choices=['sigma', 'rho', 'beta'],
        required=True,
        help='Parameter to sweep',
    )
    sweep_parser.add_argument(
        '--range', nargs=2, type=float, required=True, help='Parameter range (min max)'
    )
    sweep_parser.add_argument(
        '--steps', type=int, default=50, help='Number of parameter values'
    )
    sweep_parser.add_argument(
        '--x0', type=float, default=1.0, help='Initial X coordinate'
    )
    ...
    sweep_parser.add_argument(
        '--sim-steps', type=int, default=10000, help='Simulation steps'
    )
    sweep_parser.add_argument(
        '--method',
        choices=['euler', 'rk4', 'adaptive', 'dopri5'],
    ...
    # Bifurcation command
    bifurcation_parser = subparsers.add_parser(
        'bifurcation', help='Bifurcation analysis'
    )
    bifurcation_parser.add_argument(
        '--parameter',
        choices=['sigma', 'rho', 'beta'],
        required=True,
    ...
    bifurcation_parser.add_argument(
        '--points', type=int, default=200, help='Number of points'
    )
    bifurcation_parser.add_argument(
        '--x0', type=float, default=1.0, help='Initial X coordinate'
    )
    ...
    bifurcation_parser.add_argument(
        '--sim-steps', type=int, default=20000, help='Simulation steps'
    )
    ...
    analysis_parser.add_argument(
        '--type',
        choices=['lyapunov', 'poincare', 'sensitivity'],
        required=True,
        help='Analysis type',
    )
    analysis_parser.add_argument(
    ...
    analysis_parser.add_argument(
        '--steps', type=int, default=50000, help='Number of steps'
    )
...
def cmd_simulate(args) -> None:
    ...
    config = SimulationConfig(
        dt=args.dt, num_steps=args.steps, integration_method=args.method
    )

    # Run simulation
    print(
        f"Running simulation with parameters: σ={params.sigma}, ρ={params.rho}, β={params.beta}"
    )
    print(
        f"Initial conditions: ({initial_conditions.x:.3f}, {initial_conditions.y:.3f}, {initial_conditions.z:.3f})"
    )
...
def cmd_sweep(args) -> None:
    ...
    initial_conditions = InitialConditions(x=args.x0, y=args.y0, z=args.z0)
    config = SimulationConfig(
        dt=args.dt, num_steps=args.sim_steps, integration_method=args.method
    )
    ...
    print(
        f"Running parameter sweep: {args.parameter} from {args.range[0]} to {args.range[1]}"
    )
...
def cmd_bifurcation(args) -> None:
    ...
    print(
        f"Running bifurcation analysis: {args.parameter} from {args.range[0]} to {args.range[1]}"
    )
...
def cmd_analysis(args) -> None:
    ...
    if args.type == 'lyapunov':
        ...
        lyapunov_exponents = simulator.estimate_lyapunov_exponent(
        ...
        if args.output:
            with open(args.output, 'w') as f:
                ...
                f.write(
                    f"Parameters: σ={params.sigma}, ρ={params.rho}, β={params.beta}\n"
                )
                f.write(
                    f"Initial conditions: ({initial_conditions.x}, {initial_conditions.y}, {initial_conditions.z})\n"
                )
        ...
        sensitivity_data = simulator.sensitivity_analysis(
            initial_conditions, config=config
        )
