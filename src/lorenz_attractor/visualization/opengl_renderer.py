...
class OpenGLRenderer:
    ...
    def _generate_colors(self, result: SimulationResult, color_mode: str) -> np.ndarray:
        ...
        if color_mode == 'time':
            ...
            vel_magnitudes = np.append(
                vel_magnitudes, vel_magnitudes[-1]
            )  # Add last point

            vel_norm = (vel_magnitudes - vel_magnitudes.min()) / (
                vel_magnitudes.max() - vel_magnitudes.min()
            )
    ...
    def _calculate_mvp_matrix(self) -> np.ndarray:
        ...
        # Rotation matrices
        cos_x, sin_x = math.cos(math.radians(self.camera_angle_x)), math.sin(
            math.radians(self.camera_angle_x)
        )
        cos_y, sin_y = math.cos(math.radians(self.camera_angle_y)), math.sin(
            math.radians(self.camera_angle_y)
        )
