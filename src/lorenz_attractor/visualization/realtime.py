...
class RealtimeVisualizer:
    ...
    def start_pygame_visualization(
        self,
        initial_conditions: InitialConditions,
        config: SimulationConfig,
    ...
    def start_moderngl_visualization(
        self,
        initial_conditions: InitialConditions,
        config: SimulationConfig,
        window_size: Tuple[int, int] = (1200, 800),
        ...
        while self.is_running:
            ...
            if len(self.trajectory_buffer) > 1:
                ...
                vao = ctx.vertex_array(
                    program,
                    [(vbo, '3f', 'in_position'), (alpha_buffer, '1f', 'in_alpha')],
                )
...
class StreamingVisualizer:
    ...
    def start_streaming(
        ...
        self.stream_thread = threading.Thread(
            target=self._stream_worker, args=(initial_conditions, config)
        )
